...
//			$con = mysql_connect("localhost","Username","Password");	// TODO Fill in this line with the database login details & uncomment.
				...
				if (!$con){	die('Could not connect: ' . mysql_error());	}
					...
					if($usernameID  == $creatorID){
						...
						// Add points and experience to the root user..
						mysql_query("UPDATE USERS SET POINTS = POINTS + 1 WHERE ID = '$creatorID'");
						mysql_query("UPDATE USERS SET EXP = EXP + 1 WHERE ID = '$creatorID'");
					}
					echo "Success";
					mysql_close($con);
				}
